...
fn main() {
    ...
    #[cfg(target_os = "macos")]
    {
        let clang_llvm_version = "clang+llvm-11.0.0-x86_64-apple-darwin";
        let status = Command::new("curl")
            .args(&[
                "-L",
                "-s",
                "-O",
                &format!("https://github.com/llvm/llvm-project/releases/download/llvmorg-11.0.0/{}.tar.xz", clang_llvm_version),
            ])
            .current_dir(&out_dir)
            .status()
            .expect("Failed to start");
